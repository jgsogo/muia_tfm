    ...
    while (it != endit) {
        if (fs::is_regular_file(*it) && it->path().extension() == ".txt") {
            ...
            try {
                ...
                ofstream fori; fori.open(directory + fileid + "_original.dot"); original.print(fori); fori.close();
                wn::unl_graph google;
                auto google_graph = parse_graph(filename, "Google", wnet, google);
                ofstream fgoo; fgoo.open(directory + fileid + "_google.dot"); original.print(fgoo); fgoo.close();
                wn::unl_graph yandex;
                auto yandex_graph = parse_graph(filename, "Yandex", wnet, yandex);
                ofstream fyan; fyan.open(directory + fileid + "_yandex.dot"); original.print(fyan); fyan.close();

                if (ori_graph && google_graph) {
                    graph_dist dist_original(original, google, "- Google/Original: ");
                    for (auto tol_synset = 0; tol_synset < 10; ++tol_synset) {
                        auto tol_synset_value = 0.1f*tol_synset;
                        for (auto tol_relation = 0; tol_relation < 10; ++tol_relation) {
                            auto tol_relation_value = 0.1f*tol_relation;
                            fout << it->path().filename().string() << ";\t"
                                << "google" << ";\t"
                                << "sp" << ";\t"
                                << tol_synset_value << ";\t"
                                << tol_relation_value << ";\t"
