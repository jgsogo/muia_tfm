...
claramente expuesto, se ha decidido crear un modelo desde cero utilizando algunas ideas
vistas durante la revisión del estado del arte e incorporando la posibilidad de indicar un
umbral de flexibilidad en la comparación entre conceptos.
...
de distancia entre grafos UNL puesto que estamos sustituyendo las UWs por los *synsets*. Esta
sustitución, sin embargo, no supone ninguna pérdida de generalidad puesto que la metodología
utilizada puede ser aplicada de la misma forma en cualquier jerarquía de conceptos con
relaciones de hiponimia.

El modelo propuesto satisface los siguientes objetivos:

 * Ofrecer una medida de similaridad entre grafos conceptuales cuyo valor sea absoluto en una escala
   en el intervalo :math:`[0,1]`, donde :math:`0` indica mínima similaridad y el valor :math:`1`
   que son idénticos. No se plantea el estudio sobre si esta medida cumple las propiedades de
   una distancia y por lo tanto podría ser tratada como tal, así que preferimos hablar de
   similaridad.
   
 * Maximizar la similaridad calculada entre grafos: el algoritmo debe buscar la combinación que
   ofrezca la mínima distancia entre los grafos a comparar, el cálculo propuesto realiza una
   búsqueda exhaustiva por lo que está garantizado que encuentra el máximo global [#]_.
...
.. [#] Como se expondrá más adelante esta búsqueda exhaustiva resulta en muchos casos ineficiente
